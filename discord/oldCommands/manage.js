...
module.exports = {
    ...
    execute(message, args, client) {
      ...
      const reply = sendDetails(config.bot.token1, args[0], args[1], args[2], avatar, discordTag)
...
async function sendDetails(token, action, user, input, avatar, username) {
    return new Promise((resolve, reject) => {
        if(action == "create"){
          input = (input + "," + avatar + "," + username)
        }
        console.log(`http://localhost:8080/api/v1/manage/token=${token}&action=${action}&user=${user}&input=${input}`)
  
        http.get(`http://localhost:8080/api/v1/manage/token=${token}&action=${action}&user=${user}&input=${input}`, (resp) => {
          let data = '';
